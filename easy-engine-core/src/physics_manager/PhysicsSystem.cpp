...
auto PhysicsSystem::Update(float dt) -> void {
  for (auto const entity : this->entities_) {
    auto maybe_rigid_body_component =
        this->world->GetComponentForEntity<ecs::component::RigidBodyComponent>(
            entity);
    auto maybe_transform_component =
        this->world->GetComponentForEntity<ecs::component::TransformComponent>(
            entity);

    if (maybe_rigid_body_component.has_value() &&
        maybe_transform_component.has_value()) {
      auto transform = maybe_transform_component.value();
      const auto rigid_body = maybe_rigid_body_component.value();

      this->physics_manager_->Register(rigid_body, transform);
      // Register rigid body in physics world (in PhysicsManager)
