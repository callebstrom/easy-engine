...
namespace easy_engine {
  namespace resource_manager {
    ...
    struct ResourceManager3D::Impl {
      ...
      void LoadTexture(std::string mesh_path, const aiScene* scene, const aiMesh* mesh_, std::vector<resource::Texture*>& textures, size_t& type_index, resource::TextureType texture_type) {
        aiString* texture_path = new aiString;

        aiGetMaterialTexture(scene->mMaterials[mesh_->mMaterialIndex], (aiTextureType)texture_type, 0, texture_path, 0, 0, 0, 0, 0, 0);
        if (auto texture = scene->GetEmbeddedTexture(texture_path->C_Str())) {
          LoadEmbeddedTexture(texture, mesh_, textures, type_index, texture_type);
        }
        else {
          LoadExternalTexture(mesh_path, scene, mesh_, textures, type_index, texture_type);
        }
      }

      void LoadEmbeddedTexture(const aiTexture* texture_, const aiMesh* mesh_, std::vector<resource::Texture*>& textures, size_t& type_index, resource::TextureType texture_type) {
        std::string cache_key = mesh_->mName.C_Str() + std::string(texture_->mFilename.C_Str());
      ...
      void LoadExternalTexture(std::string mesh_path, const aiScene* scene, const aiMesh* mesh_, std::vector<resource::Texture*>& textures, size_t& type_index, resource::TextureType texture_type) {
        ...
        if (texture_index_buffer->length > 0) {
          ...
          if (texture->raw == nullptr) {
            boost::filesystem::path assume_same_dir(texture_path);
            texture = ResourceManager::LoadTextureFromFile(assume_same_dir.filename().string());
          }
        ...
        for (size_t i = 0; i < scene->mNumMeshes; i++) {
          ...
          if (this->p_impl_->material_cache.HasResource(cache_key)) {
            ...
            if (flags & LOAD_MATERIALS) {
              ...
              auto ret = material_->Get(AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0), textureName);
            ...
            if (flags & LOAD_TEXTURES) {
              ...
              auto texture_load = [this, scene, mesh_, file_path, material](std::mutex& texture_vector_mutex, std::vector<resource::Texture*>& textures) -> void {
                this->p_impl_->LoadTexture(file_path, scene, mesh_, textures, material->diffuse_texture_index, resource::TextureType::kDiffuse);
                this->p_impl_->LoadTexture(file_path, scene, mesh_, textures, material->specular_texture_index, resource::TextureType::kSpecular);
                this->p_impl_->LoadTexture(file_path, scene, mesh_, textures, material->emissive_texture_index, resource::TextureType::kEmissive);
              };
