...
public:
  ViewportControls() {
    ManagerLocator::event_manager->Subscribe(
      event_manager::EventType::kKeyboard, this);
  }
      ...
      if (event->action == Action::kPressed) {
        for (auto entity : this->entities_) {
          ...
          case Key::kD:
            ...
            break;
          case Key::kR: {
            auto rotation_direction =
              event->modifiers == Modifier::kControl ? -1.f : 1.f;
            transform->RotationAdd(rotation_direction * positiveFactor,
              ecs::component::kPivotZ);
            break;
          }
          case Key::kUp:
            transform->TranslationAdd(0, positiveFactor * translateFactor, 0);
            break;
          case Key::kDown:
            transform->TranslationAdd(0, negativeFactor * translateFactor, 0);
            break;
          }
        }
      }
...
private:
  ...
  bool textures = true;
  std::optional<ecs::component::TextureComponent*> cached_texture_component =
    std::nullopt;
};
...
public:
  ...
  void Update(float dt) override {
    for (auto const& entity : this->entities_) {
      auto maybe_text_area =
        this->world->GetComponentForEntity<ui::component::TextAreaComponent>(
          entity);
      if (maybe_text_area.has_value()) {
        ...
        const auto shader = this->shader_manager_->LoadShaderBySource(
          source, shader_manager::ShaderType::kPixel);
        if (shader) {
          auto new_pipeline = this->shader_manager_->CreateShaderPipeline();
          this->shader_manager_->AttachShader(pipeline->GetVertexShader(),
            new_pipeline);
...
public:
  ...
  virtual void OnInit() override {
    ...
    world->AddSystem<ui::component::TextAreaComponent>(
      new ShaderEditor(this->shader_manager));
    ...
    this->resource_manager_3d
      ...
      .then([=](resource_manager::LoadFuture
        future_resource_collection) mutable {
          ...
          auto mesh_component = CreateRef<ecs::component::MeshComponent>();
          auto texture_component =
            CreateRef<ecs::component::TextureComponent>();
          auto material_component =
            CreateRef<ecs::component::MaterialComponent>();

          mesh_component->sub_meshes =
            new std::vector<resource::Mesh*>(resource_collection.meshes);
          texture_component->textures = new std::vector<resource::Texture*>(
            resource_collection.textures);
          material_component->materials = new std::vector<resource::Material*>(
            resource_collection.materials);

          this->world->AddComponent<ecs::component::MeshComponent>(
            entity, mesh_component);
          this->world->AddComponent<ecs::component::TextureComponent>(
            entity, texture_component);
          this->world->AddComponent<ecs::component::MaterialComponent>(
            entity, material_component);
          this->world->AddComponent<ecs::component::TransformComponent>(
    ...
    auto point_light_transform =
      CreateRef<ecs::component::TransformComponent>();
    point_light_transform->Translation(0.f, .5f, 10.f);

    auto lamp_entity = this->world->CreateEntity();
    this->world->AddComponent(lamp_entity, point_light_component);
    ...
    sun_transform->Translation(0.f, 2.f, 10.f);

    auto sun_entity = this->world->CreateEntity();
    this->world->AddComponent<ecs::component::LightComponent>(sun_entity,
      sun_component);
    this->world->AddComponent<ecs::component::TransformComponent>(
      sun_entity, sun_transform);
  ...
  void SetupWindow() {
    ...
    auto window_component = CreateRef<ui::component::WindowComponent>();
    window_component->height = 800;
    window_component->width = 600;
    window_component->title = "Shader editor";
    auto shader_editor_transform_component =
      CreateRef<ecs::component::TransformComponent>();
    shader_editor_transform_component->Translation(100, 100, 0);

    world->AddComponent<ecs::component::TransformComponent>(
      shader_editor_window, shader_editor_transform_component);
    world->AddComponent<ui::component::WindowComponent>(shader_editor_window,
      window_component);

    auto text_area = world->CreateEntity();
    auto pixel_shader =
      this->shader_manager->GetAttachedPipeline()->GetPixelShader();
    auto source = pixel_shader->GetSource();
    auto text_area_component =
      CreateRef<ui::component::TextAreaComponent>(800, 2000, 40000, source);
    auto text_area_transform_component =
      CreateRef<ecs::component::TransformComponent>();
    auto window_parent_component =
      CreateRef<ecs::component::ParentEntityComponent>();
    window_parent_component->entity = shader_editor_window.entity;

    world->AddComponent<ecs::component::TransformComponent>(
      text_area, text_area_transform_component);
    world->AddComponent<ui::component::TextAreaComponent>(text_area,
      text_area_component);
    world->AddComponent<ecs::component::ParentEntityComponent>(
      text_area, window_parent_component);
  }
};
